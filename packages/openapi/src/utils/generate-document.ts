...
export function generateDocument(
  ...
  const bannerContent: Record<string, unknown> = {
    title: options.title,
  ...
  // Generate frontmatter without comments
  const banner = `---\n${dump(bannerContent).trim()}\n---`;
  
  if (banner.length > 0) out.push(banner);

  // Add auto-generated comment if enabled (as MDX comment below frontmatter)
  if (addGeneratedComment !== false) {
    ...
    if (typeof addGeneratedComment === 'string') {
      // Use the custom comment
      commentContent = addGeneratedComment;
    }

    // Escape any MDX comment closing sequences in the comment content
    commentContent = commentContent.replace(/-->/g, '--\\>');
    
    // Format as MDX comment below the frontmatter
