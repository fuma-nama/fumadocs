...
type ShallowDoc<Doc extends BaseDoc> = Pick<
  Doc,
  '_id' | '_type' | '_updatedAt' | 'title' | 'slug' | 'description'
>;
...
export function createSanitySource<Doc extends BaseDoc>(
  ...
  type SanityVirtualFile = VirtualFile<{ pageData: DocToPage<Doc>; metaData: MetaData }>;
  ...
  let fileCache = new Map<string, SanityVirtualFile>();
  ...
  function toVirtualFile(file: ShallowDoc<Doc>): SanityVirtualFile {
  ...
  return {
    ...
    async files() {
      const data = await sanityFetch<ShallowDoc<Doc>[]>(
        `*[_type == $docType]{ _id, _type, _updatedAt, title, slug, description }`,
        {
          docType,
        },
      );

      const next = new Map<string, SanityVirtualFile>();
