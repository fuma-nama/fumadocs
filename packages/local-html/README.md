...
Trust model: content is transformed, not sanitized. Nothing that executes survives — `<script>`, `<style>`, `<iframe>`, `<object>`, `<embed>`, `<form>`, inline event handlers and `javascript:` URLs are always dropped — but the remaining markup is rendered as-is, and remote resources it references (images above all) are fetched by everyone who opens the page. Treat the content directory like you treat your Markdown content: local files you trust.
