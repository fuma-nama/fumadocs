...
`,
    ...
    expect(dereference(pet.allOf[1])).toEqual({
      type: 'object',
      properties: {
        owner: { type: 'string' },
      },
...
`,
    ...
    expect(dereference(proxied.components.schemas.Pet)).toMatchObject({
      type: 'object',
      properties: {
        name: { type: 'string' },
      },
  ...
  test('keeps internal refs that traverse through another ref resolvable', async () => {
    ...
    expect(dereference(proxied.components.schemas.Wrapper.properties.value)).toEqual({
      type: 'string',
    });
  });
});
