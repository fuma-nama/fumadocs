...
const defaultOptions: Required<Pick<SampleOptions, 'skipReadOnly' | 'maxSampleDepth'>> = {
  skipReadOnly: false,
  maxSampleDepth: 15,
};
...
function inferType(schema: JsonSchema): TypeName | undefined {
...
function sampleArray(
  ...
  const itemSchemaGetter = (i: number): JsonSchema => {
    if (Array.isArray(items)) {
      return items[i] ?? {};
    } else {
      return items ?? {};
...
type ResolvedOptions = typeof defaultOptions & SampleOptions;

function inferExample(schema: Exclude<JsonSchema, boolean>): unknown | undefined {
...
function tryInferExample(schema: JsonSchema): TraverseResult | undefined {
  if (typeof schema === 'boolean') return;
...
function allOfSample(
  into: Exclude<JsonSchema, boolean>,
  children: readonly JsonSchema[],
  options: ResolvedOptions,
  context: TraverseContext | undefined,
): TraverseResult {
...
const typeSamplers: Record<
  string,
  (schema: JsonSchema, options: ResolvedOptions, context: TraverseContext | undefined) => unknown
> = {
  array: sampleArray,
...
function traverseOneOrAnyOf(
  parent: Exclude<JsonSchema, boolean>,
  selectedSubSchema: JsonSchema,
  options: ResolvedOptions,
  context: TraverseContext | undefined,
): TraverseResult {
...
function traverse(
  ...
  if (typeof schema.$ref === 'string') {
    ...
    const referenced = (schema as { '$ref-value': JsonSchema | undefined })['$ref-value'];
...
export function sample(schema: JsonSchema, options?: SampleOptions): unknown {
