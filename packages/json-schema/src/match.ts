...
export function matchesType(value: unknown, type: JsonType): boolean {
 ...
 */
export function matches(schema: JsonSchema, value: unknown): boolean {
  return match(schema, value, new Set());
}

function match(schema: JsonSchema, value: unknown, seen: Set<object>): boolean {
  ...
  if (resolved.type !== undefined) {
    ...
    if (types.every((type) => !matchesType(value, type))) return false;
  }
