...
- `fumadocs-openapi`: `createOpenAPIPage()` with your own components, and the hooks of a page: `useOpenAPI()`, `useComponents()`, `useServer()`.
...
`generateFiles()` reads and writes files, so the package entry ships a stubbed build under the `browser` condition. Client components can import `createOpenAPIPage()` and the hooks without pulling `node:fs` into the bundle.
