...
Most themes primarily swap **color tokens** (backgrounds, borders, accents, etc.) while keeping the same overall UI structure.

**`solar` and `mintlify` are different:** they are **layout presets** in addition to color presets. That means they can change not just the palette, but also **layout and component styling** (e.g. nav/header presentation, spacing, typography treatment, and other structural UI details) to better match those design systems.
<Tabs items={['neutral', 'black', 'vitepress', 'dusk', 'catppuccin', 'ocean', 'purple', 'solar', 'emerald', 'ruby', 'aspen']}>

<Tabs items={['neutral', 'black', 'vitepress', 'dusk', 'catppuccin', 'ocean', 'purple', 'solar', 'mintlify']}> <Tab value='neutral'>
...
![Solar (layout + colors)](/themes/solar.png)
...
![Mintlify (layout + colors)](/themes/mintlify.png)
