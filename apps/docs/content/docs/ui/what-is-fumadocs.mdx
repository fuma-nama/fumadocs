...
Fumadocs was created because **Fuma** <span className='text-fd-muted-foreground'>(the creator)</span> wanted for a more customisable experience for building docs, to be a docs framework that is not opinionated, **a "framework" that you can break**.

## Philosophy

**Less Abstraction:** Fumadocs expects you to write code and cooperate with rest of your software.
While most frameworks are configured with a configuration file, they usually lack flexibility when you attempt to complete something more advanced.
