...
<Callout type="warn" title="Order matters">
  Import it **before** `fumadocs-ui/css/preset.css`. The preset contains a `@plugin` directive, and
  any `@source` rules imported after it are ignored — the layout would silently render unstyled.
</Callout>
