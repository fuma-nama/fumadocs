...
Tabs are generated from the page tree, you can mark folders as [Root Folders](/docs/page-conventions#root-folder) by creating a `meta.json` file in the folder:
...
Tabs are grouped by [root type](/docs/page-conventions#root-type), with a dropdown for each root folder on the current page's path.

<Files>
  <Folder name="v2 (root: version)" defaultOpen>
    <Folder name="framework (root: true)" defaultOpen>
      <File name="quickstart.mdx" className="text-fd-primary! bg-fd-primary/10!" />
    </Folder>
    <Folder name="headless (root: true)">
      <File name="quickstart.mdx" />
    </Folder>
  </Folder>
  <Folder name="v1 (root: version)">
    <Folder name="framework (root: true)">
      <File name="quickstart.mdx" />
    </Folder>
  </Folder>
</Files>

On `v2/framework/quickstart.mdx`, the sidebar shows a dropdown for the version and another for the section:

<div className='flex justify-center items-center *:max-w-[360px] bg-linear-to-br from-fd-primary/10 rounded-xl border'>

    ![Tabs Groups](/docs/sidebar-tabs-groups.png)

</div>

Selecting an option navigates to the same page in that root folder, or its index page when the page doesn't exist there. See [Versioning](/docs/navigation#versioning) for a complete example.
