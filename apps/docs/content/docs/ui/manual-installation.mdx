...
If you deploy your nextjs app using Docker, you need to add the `source.config.ts` file to the WORKDIR in the Dockerfile.
The following snippet is taken from the official nextjs Dockerfile [ exmaple ](https://github.com/vercel/next.js/blob/canary/examples/with-docker/Dockerfile)
...
> This prevents the docker build error `Could not resolve "/app/source.config.ts"`
