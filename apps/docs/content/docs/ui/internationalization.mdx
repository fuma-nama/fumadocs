...
Fumadocs is not a full-powered i18n library, it's up to you when internationalizing the nest of your app.
...
> See [available options](/docs/headless/internationalization) for further customisations.
...
<Callout title="Using your own middleware?">
    ...
    When using custom middleware, make sure its behaviour align with the [`hidePrefix`](/docs/headless/internationalization#hide-locale-prefix) option you set in your i18n config.

</Callout>

### Routing

Create a `/app/[lang]` folder, and move your routes from `/app` folder (e.g. `page.tsx`, `layout.tsx`) to the folder, except route handlers.
...
// make sure `locale` is consistent with your i18n config [!code ++:10]
const locales = [
  {
    name: 'English',
    locale: 'en',
