...
<auto-type-table name="AutoTypeTableExample" type='export interface AutoTypeTableExample {
...
You cannot use this in a client component, instead, try the [build-time MDX integration](/docs/ui/typescript#mdx-integration) instead.
...
See [TypeScript DocGen](/docs/ui/typescript) for more usages.
...
You can change the compiler settings from [`createGenerator()`](/docs/ui/typescript).
