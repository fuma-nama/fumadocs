...
- **Composable:** Layout components can easily manage their position & size, preferably in place.
- **Flexible:** Don't rely on fixed values/heights, any external component (e.g. AI Chat) can be corporated into the layout.
- **Cohesive:** Layout components can reflect on others' changes, e.g. animating sidebar collapsing.
- **Predictable:** Layout properties should not be scattered, the ultimate result can be easily predicted from source code.

And most importantly, maximize browser compatibility, which means it cannot leverage the latest CSS features.
...
#nd-docs-layout {
  ...
  --fd-docs-row-2: calc(var(--fd-docs-row-1) + var(--fd-header-height) 0px);
