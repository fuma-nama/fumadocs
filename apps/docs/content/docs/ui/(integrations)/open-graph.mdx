...
> We need to append `image.webp` to the end of slugs so that we can access it via `/docs-og/my-page/image.webp`.
...
export async function generateMetadata({
  ...
  const image = ['/docs-og', ...slug, 'image.webp'].join('/');
  return {
...
generateOGImage({
  // Use `fs` or `fetch` to load the font 
  fonts: [robotoArrayBuffer],
});
```
