...
```ts tab="React Router" title="app/routes/llms-mdx.ts"
...
```ts tab="Tanstack Start" title="src/routes/llms[.]mdx.$.ts"
...
Step 1: Add your AI API key to `.env` file:

```tsx title=".env"
INKEEP_API_KEY = 'your-inkeep-api-key';
```

Step 2: Import and add the `AISearchTrigger` component:

```tsx title="app/docs/[[...slug]]/page.tsx"
import { AISearchTrigger } from '@/components/search'; // Import the component
```

Step 3: Add the `<AISearchTrigger />` component (can be anywhere in the layout, usually at the root layout or docs layout):

```tsx title="app/docs/[[...slug]]/page.tsx"
<DocsPage toc={page.data.toc} full={page.data.full}>
  ... // your existing content
  <AISearchTrigger /> // Add this line
</DocsPage>
```

### AI Model

By default, it's configured for [Inkeep AI](https://inkeep.com) using Vercel AI SDK. Update the configurations in `useChat` and `/api/chat` route to connect to your own AI model instead.
