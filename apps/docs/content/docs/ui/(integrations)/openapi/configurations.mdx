...
The Input OpenAPI schema.

It can be the schema content (as JavaScript Object), a file path, or URL.
The schema is dynamically fetched when the `APIPage` component renders.
...
<Callout type='warn' title='For Vercel'>
    ...
    If it relies on the file system, ensure the page **will not** be re-rendered after build.
