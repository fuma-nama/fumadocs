...
For Fumadocs MDX, a `source.config.ts` config file is included. Make sure to read [Introduction](/docs/mdx) to learn how it handles your content,
and customise different configurations like frontmatter schema.
...
And `fumadocs-mdx` can be a useful library to handle MDX content in Next.js similar to Content Collections.
