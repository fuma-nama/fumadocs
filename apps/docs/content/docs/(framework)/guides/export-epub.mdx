...
Create a route handler to serve the EPUB:
...
Consider protecting `/export/epub` in production. For example, require an `Authorization: Bearer <secret>` header and set `EXPORT_SECRET` in your environment, like the route created by Fumadocs CLI.
