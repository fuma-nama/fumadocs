...
`@fumadocs/story` exports `<StoryProvider />`, `useStory()` and `useStoryArgs()`, which provide the state of a story without UI:
...
'use client';
import { StoryProvider, useStory, useStoryArgs, type WithControlProps } from '@fumadocs/story';
...
```npm
npx @fumadocs/cli add story/controls
```
