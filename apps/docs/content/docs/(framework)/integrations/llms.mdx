...
The Markdown of a single page, for AI agents. Pages are served under `/llms.mdx/docs/<slugs>/content.md`, the route prefix is a constant in `lib/shared.ts`.

On Tanstack Start, the route lives under the docs route instead: `/docs/<slugs>.md`.
...
The tools come from `fumadocs-core/mcp`: `registerSourceTools()` adds `list_pages` and `get_page`, `registerSearchTool()` adds `search`.
