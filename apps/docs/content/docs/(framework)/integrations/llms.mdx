...
The AI prompts name the page by the URL the reader is on, without query and hash, so a site served under a base path or behind a rewrite hands out a URL that resolves.
Pass `pageUrl` to name a canonical URL instead.
