---
title: Without RSC
descirption: Setup guide for non-RSC environments.
---
...
It shares a different type from your original source, explicit handling of OpenAPI pages is necessary.

For example, update all references to `source`'s pages, including utils like `getLLMText()`:
...
Make sure to check the types before continuing.
...
const serverLoader = createServerFn({
  ...
  .handler(async ({ data: slugs }) => {
    ...
    // different result for OpenAPI pages [!code ++:12]
    if (page.data.type === 'openapi') {
      return {
        type: 'openapi',
