...
It takes the same options as `createOpenAPIPage()`, with a Shiki factory.

```tsx title="components/api-page.tsx"
'use client';
import { createOpenAPIPage } from 'fumadocs-openapi/ui'; // [!code --]
import { createOpenAPIPageBase } from '@/components/api/page'; // [!code ++]
import { defaultShikiFactory } from 'fumadocs-core/highlight/shiki/full'; // [!code ++]

export const OpenAPIPage = createOpenAPIPage(); // [!code --]
export const OpenAPIPage = createOpenAPIPageBase({ shiki: defaultShikiFactory }); // [!code ++]
```

The sections below install parts of the UI instead, and use them in the built-in page.
...
```npm
npx @fumadocs/cli add fumadocs/openapi/operation
```
