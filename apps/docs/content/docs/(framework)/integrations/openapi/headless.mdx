...
`createOpenAPIPage()` takes your components and returns `<OpenAPIPage />`, which accepts the props of generated pages.

```tsx title="components/api-page.tsx"
'use client';
import { type CodeBlockProps, createOpenAPIPage } from 'fumadocs-openapi';
...
`Markdown`, `CodeBlock` and `Heading` are filled in when you leave them out, rendering Markdown through Remark and code through the `shiki` option.
