---
title: Takumi
description: Framework agnostic and fast OG image generation.
---
...
To avoid bundling the native binary and allow `require()` calls to function properly, additional configuration is required depending on your bundler.
...
```ts title="vite.config.ts" tab="Nitro"
