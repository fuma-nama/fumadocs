...
`createAsyncAPIPage()` takes your components and returns `<AsyncAPIPage />`, which accepts the props of generated pages.
...
To render operations yourself, mount `<AsyncAPIProvider document={bundled} components={...} />` in place of `createAsyncAPIPage()`.
