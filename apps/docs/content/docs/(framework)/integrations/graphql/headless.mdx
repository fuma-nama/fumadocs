...
`createGraphQLPage()` takes your components and returns `<GraphQLPage />`, which accepts the props of generated pages.
...
To render operations yourself, mount `<GraphQLProvider sdl={sdl} components={...} />` in place of `createGraphQLPage()`.
