...
It pairs a Markdown compiler with [`@fumadocs/local-content`](/docs/integrations/content/local-content), which handles the file scanning, schema validation and hot reload described below.
