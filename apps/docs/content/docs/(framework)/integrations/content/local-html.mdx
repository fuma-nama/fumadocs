...
- Processing is a transform, not sanitization: `<script>`, `<style>`, `<iframe>`, `<object>`, `<embed>` and inline event handlers are always dropped, but the remaining markup is rendered as-is — the content directory must contain files you trust, like your Markdown content.
