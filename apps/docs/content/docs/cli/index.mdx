...
Routes are placed under the constants of `lib/shared.ts` (`docsRoute`, `docsContentRoute`, `docsImageRoute`), define them first to customize the URLs. Features that add routes also add URL helpers to `lib/source.ts`, like `getPageMarkdownUrl(page)` and `getPageImageUrl(page)`.
