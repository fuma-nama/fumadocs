...
If your source changes over time, call `revalidate()` to clear the cache:

```ts
source.revalidate();
```

For [multi-source](/docs/headless/source-api/source#multiple-sources) setups, you can revalidate a specific source:
