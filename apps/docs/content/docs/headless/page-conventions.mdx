...
For built-in content sources like Fumadocs MDX, you can use [the `icon` handler](/docs/headless/source-api#icons) from Source API.
...
When users are browsing the pages under a root folder, only the child items of the folder will be visible.
...
To allow users switching between different roots, implement a component that navigates to a page under the root folders.
On Fumadocs UI, this is available via the [Root Toggle](/docs/ui/components/root-toggle) component.
