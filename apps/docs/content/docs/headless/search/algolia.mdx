...
sync(client, {
  documents: indexes, // search indexes, may be provided by your content source [!code highlight]
});
```
...
To implement a search UI, consider [Fumadocs UI Search Dialog](/docs/ui/search#algolia), or build your own.

In addition, the headless search client of Fumadocs can handle state management on React.
