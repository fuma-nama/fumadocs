...
For instance, you can define and configure collections by creating a config file: `source.config.ts`.
...
`defineDocs` defines the required collection for Fumadocs to work.
It includes all Markdown/MDX and JSON files under your `content/docs` directory.
