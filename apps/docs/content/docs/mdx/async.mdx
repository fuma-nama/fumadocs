...
<Callout title="Next.js Only">
  At the moment, Async Mode is only a Next.js feature. Vite has native support
  for lazy compilation, hence it might not be needed at all.
</Callout>
...
Async Mode allows on-demand compilation of Markdown and MDX content by moving the compilation process from build time to server runtime.
