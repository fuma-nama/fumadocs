...
Using the [Macro API](/docs/mdx/macro) instead? Import the collection on the client and render with `getPage` / `body` (see [Macro API](/docs/mdx/macro#browser)).

## Examples

The examples are for non-RSC usage. Prefer RSC when possible for better client performance.
