...
**For example:** A small CSS change can still break hacky CSS customizations. We can only ensure backward compatibility for the ones scoped by CSS selectors, or exposed options:
...
Same as the solution described in [this post about UI Customization](/blog/customize-ui#the-solution).

### A Good Example

For lighter customizations like nuqs did, a PR to migrate from old versions [#1056](https://github.com/47ng/nuqs/pull/1056) can reduce code on user land and migration is simple.

### The Problems

When your hacks are heavier, migration is much harder combined with the tech debt of your codebase.
...
I think the CLI approach is correct, it'll be the primary way for advanced customization.
